...
test('H9 Massive E2E Runthrough: Sem 1 to 6 Validation', async ({ page, request, seededRun }) => {
  ...
  await enterMarksViaApi(request, secondaryOfferingId, 'tt1', tt1EntriesB, clMarkSess.csrfToken, { lock: true, evaluatedAt: getEvalDate() })
  ...
  try {
    ...
    await enterAttendanceViaApi(request, primaryOfferingId, attEntries, attSession.csrfToken, { lock: true })
  ...
  for (let sem = 2; sem <= 6; sem++) {
    ...
    for (const evalDef of stageGatedEvals) {
      ...
      try {
        ...
        await enterMarksViaApi(request, semOfferingId, evalDef.kind, entries, ownerSess2.csrfToken, {
          lock: false,
          evaluatedAt: getEvalDate(),
        })
    ...
    try {
      const attEntries = generateAttendancePayload(sectionAStudents, specialStudentIds, trajectoryMap)
      const ownerSess = await freshOwnerSession(request, ownerIdentifier)
      await enterAttendanceViaApi(request, semOfferingId, attEntries, ownerSess.csrfToken, { lock: false })

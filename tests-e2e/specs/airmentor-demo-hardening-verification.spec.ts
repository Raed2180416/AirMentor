...
const EVIDENCE_ROOT = process.env.AIRMENTOR_DEMO_HARDENING_EVIDENCE_DIR
  ?? path.join(process.cwd(), 'output/playwright/airmentor-demo-hardening-verification')
...
async function screenshot(page: { screenshot(options: { path: string; fullPage?: boolean }): Promise<Buffer> }, fileName: string, fullPage = false) {
...
async function readRiskExplorer(request: RequestContext, csrfToken: string, runId: string, checkpointId: string, studentId: string) {
  ...
  const text = await response.text()
  if (!responseOk(response)) throw new Error(`Risk explorer failed for ${studentId}: ${text.slice(0, 800)}`)
  return text ? JSON.parse(text) : null
}

test.describe('AirMentor demo hardening verification', () => {
  test.describe.configure({ timeout: 900_000 })

  test('P0 proof-control Next Stage aligns with seeded evidence and preserves manual entries', async ({ page, request, seededRun }) => {
    ...
    expect(preTt1Checkpoint.simulationStageCheckpointId).toBeTruthy()
    ...
    expect(tt1Components.length).toBeGreaterThan(0)
    ...
    expect(responseOk(marksResponse), `Manual TT1 entry failed: ${marksBody.slice(0, 800)}`).toBeTruthy()
    ...
    await expect(page.locator('#system-admin-proof-controls').first()).toBeVisible({ timeout: 90_000 })
    const nextStageButton = page.locator('[data-proof-action="proof-next-stage"]').first()
    await expect(nextStageButton).toBeVisible({ timeout: 60_000 })
    await expect(nextStageButton).toBeEnabled({ timeout: 60_000 })
    await screenshot(page, '01-proof-control-before-next-stage.png', true)

    const advanceResponsePromise = page.waitForResponse(
      response => response.url().includes(`/proof-runs/${seededRun.runId}/advance`) && response.status() === 200,
      { timeout: 120_000 },
    )
    await nextStageButton.click()
    await screenshot(page, '02-proof-control-after-next-stage-click.png', true)
    const advanceResponse = await advanceResponsePromise
    expect(advanceResponse.ok()).toBeTruthy()

    await expect.poll(async () => {
      const dashboard = await readProofDashboard(request, seededRun.batchId, postEditAdminSession.csrfToken)
      return dashboard.activeRunDetail?.activeStageKey
    }, { timeout: 120_000 }).toBe('post-tt1')

    const dashboardAfter = await readProofDashboard(request, seededRun.batchId, postEditAdminSession.csrfToken)
    expect(dashboardAfter.activeRunDetail?.activeOperationalSemester).toBe(1)
    expect(dashboardAfter.activeRunDetail?.activeStageKey).toBe('post-tt1')
    await screenshot(page, '03-proof-control-after-post-tt1.png', true)
    ...
    await writeJson('01-p0-seeding-alignment.json', {
      runId: seededRun.runId,
    ...
    await writeCsv('01-p0-seeding-alignment.csv', comparisonRows)
    ...
    await expect(page.locator('[data-proof-surface="hod-proof-analytics"]').first()).toBeVisible({ timeout: 75_000 })
    await screenshot(page, '04-hod-post-tt1-proof-analytics.png', true)
  ...
  test('Semester 3 restore flow creates an active restored proof run from checkpoint state', async ({ request, seededRun }) => {
    const { session } = await loginWithApiContext(request, 'system-admin')
    await advanceProofRunToCheckpoint(request, seededRun.runId, seededRun.batchId, session.csrfToken, 3, 'post-tt1')
    const sem3Dashboard = await readProofDashboard(request, seededRun.batchId, session.csrfToken)
    expect(sem3Dashboard.activeRunDetail?.activeOperationalSemester).toBe(3)
    expect(sem3Dashboard.activeRunDetail?.activeStageKey).toBe('post-tt1')

    const restoreResponse = await request.post(apiPath(`/api/admin/proof-runs/${encodeURIComponent(seededRun.runId)}/restore-snapshot`), {
      headers: jsonHeaders(session.csrfToken),
      data: {},
    })
    const restoreText = await restoreResponse.text()
    expect(responseOk(restoreResponse), `Restore snapshot failed: ${restoreText.slice(0, 800)}`).toBeTruthy()
    const restored = restoreText ? JSON.parse(restoreText) : null
    expect(String(restored?.simulationRunId ?? '')).toMatch(/^simulation_run_/)
    expect(restored?.simulationRunId).not.toBe(seededRun.runId)

    const restoredDashboard = await readProofDashboard(request, seededRun.batchId, session.csrfToken)
    await writeJson('04-semester-3-restore-flow.json', {
      originalRunId: seededRun.runId,
      sem3BeforeRestore: sem3Dashboard.activeRunDetail,
      restored,
      activeAfterRestore: restoredDashboard.activeRunDetail,
    })
    expect(restoredDashboard.activeRunDetail?.simulationRunId).toBe(restored.simulationRunId)
  })
})

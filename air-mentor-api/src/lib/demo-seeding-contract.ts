...
export function deterministicScore(studentIndex: number, componentId: string, maxScore: number, pct: number, isSpecial = false) {
  const variedPct = isSpecial ? Math.max(0, Math.min(1, pct)) : Math.max(0, Math.min(1, pct + deterministicVariation(studentIndex, componentId, 0.045)))
  ...
  return studentIds.map((studentId, index) => {
    ...
    return {
      ...
      components: input.components.map(component => ({
        ...
        score: deterministicScore(index, component.id, component.maxScore, pct, trajectory.special),

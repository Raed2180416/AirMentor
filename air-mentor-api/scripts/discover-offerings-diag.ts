...
async function probe(identifier: string, password: string) {
  ...
  console.log(`${identifier} offerings:`, JSON.stringify(data.offerings ?? []))
}

async function main() {
  for (const id of ['rohit.menon', 'priya.raman', 'karan.naidu', 'sowmya.krishnan', 'abhinav.rao', 'neha.iyengar', 'devika.shetty']) {
    await probe(id, 'faculty1234')
  }
}

# AirMentor Skills Index

> Auto-generated from discovered skills across Windsurf, Claude Code, Cursor
> This index uses progressive disclosure: only name+description shown to agents by default.
> Full SKILL.md loaded only when skill is invoked.

**Total skills discovered:** 182
**Sources:** Windsurf, Claude Code, Cursor, Generic agent skills

## Recommended Skills for AirMentor (Devin Priority)

These skills should be preloaded for the most common tasks in this repo. See `docs/agent-map/DEVIN_AGENT_SETUP.md` and `.windsurf/AGENTS.md` for the full agent playbook.

### Frontend (React 19 + Vite)

- `vercel-react-best-practices`
- `vercel-react-view-transitions`
- `vercel-composition-patterns`
- `playwright-testing`
- `jest-vitest`
- `accessibility-wcag`
- `frontend-developer`

### Backend (Fastify + TypeScript + Python)

- `backend-engineering`
- `api-design`
- `api-testing`
- `database-engineering`
- `docker-kubernetes`
- `python-appservice-deploy`

### Data, ML, and Evaluation

- `ml-ops`
- `data-pipelines`
- `data-science`
- `analytics-engineering`

### Security, Quality, and Refactoring

- `appsec-owasp`
- `accessibility-wcag`
- `clean-code`
- `code-review-mastery`
- `absolute-simplify`

### DevOps and Deployment

- `ci-cd-pipelines`
- `azure-prepare`
- `azure-deploy`
- `terraform-iac`
- `azure-reliability`

### Documentation and Agentics

- `technical-writing`
- `internal-docs`
- `ai-agent-design`
- `mcp-builder`
- `mastra`

## MCP Plugins to Load

- `devin/filesystem` — use only when repo-map does not answer
- `devin/github-mcp-server` — PRs, issues, code search
- `devin/mcp-playwright` — browser verification
- `devin/context7` — dependency docs
- `devin/memory` — cross-session context
- `devin/fetch` — web research

## Skills by Category

### 30-60-90

- **onboarding** — >
  Scope: workspace | Source: skills | Files: 2

### 3d-audio]

- **game-audio** — >
  Scope: workspace | Source: skills | Files: 2

### 4k

- **video-creator** — >
  Scope: workspace | Source: skills | Files: 2

### [a2a

- **a2a-protocol** — >
  Scope: workspace | Source: skills | Files: 3

### [a2ui

- **a2ui** — >
  Scope: workspace | Source: skills | Files: 3

### [accessibility

- **accessibility-wcag** — >
  Scope: workspace | Source: skills | Files: 2

### [account-management

- **account-management** — >
  Scope: workspace | Source: skills | Files: 2

### [agents

- **ai-agent-design** — >
  Scope: workspace | Source: skills | Files: 2

### [agile

- **agile-scrum** — >
  Scope: workspace | Source: skills | Files: 2

### [ai-agents

- **mastra** — >
  Scope: workspace | Source: skills | Files: 3

### [analytics

- **posthog** — >
  Scope: workspace | Source: skills | Files: 3

### [android

- **android-kotlin** — >
  Scope: workspace | Source: skills | Files: 2

### [api

- **api-design** — >
  Scope: workspace | Source: skills | Files: 2
- **api-monetization** — >
  Scope: workspace | Source: skills | Files: 3

### [api-testing

- **api-testing** — >
  Scope: workspace | Source: skills | Files: 2

### [architecture

- **system-design** — >
  Scope: workspace | Source: skills | Files: 2

### [aws

- **cloud-aws** — >
  Scope: workspace | Source: skills | Files: 2

### [backend

- **backend-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### [bash

- **shell-scripting** — >
  Scope: workspace | Source: skills | Files: 2

### [bookkeeping

- **bookkeeping-automation** — >
  Scope: workspace | Source: skills | Files: 2

### [brainstorming

- **absolute-brainstorm** — >
  Scope: workspace | Source: skills | Files: 2

### [brand

- **brand-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### [budgeting

- **budgeting-planning** — >
  Scope: workspace | Source: skills | Files: 2

### [chaos-engineering

- **chaos-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### [ci-cd

- **ci-cd-pipelines** — >
  Scope: workspace | Source: skills | Files: 2

### [clean-architecture

- **clean-architecture** — >
  Scope: workspace | Source: skills | Files: 2

### [clean-code

- **clean-code** — >
  Scope: workspace | Source: skills | Files: 2

### [cli

- **cli-design** — >
  Scope: workspace | Source: skills | Files: 2

### [cloud-security

- **cloud-security** — >
  Scope: workspace | Source: skills | Files: 2

### [code-review

- **code-review-mastery** — >
  Scope: workspace | Source: skills | Files: 2

### [community

- **community-management** — >
  Scope: workspace | Source: skills | Files: 2

### [compensation

- **compensation-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### [competitive-analysis

- **competitive-analysis** — >
  Scope: workspace | Source: skills | Files: 2

### [compliance

- **regulatory-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### [computer-vision

- **computer-vision** — >
  Scope: workspace | Source: skills | Files: 2

### [content-marketing

- **content-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### [contracts

- **contract-drafting** — >
  Scope: workspace | Source: skills | Files: 2

### [copywriting

- **copywriting** — >
  Scope: workspace | Source: skills | Files: 2

### [crm

- **crm-management** — >
  Scope: workspace | Source: skills | Files: 2

### [cryptography

- **cryptography** — >
  Scope: workspace | Source: skills | Files: 2

### [customer-research

- **customer-research** — >
  Scope: workspace | Source: skills | Files: 2

### [customer-success

- **customer-success-playbook** — >
  Scope: workspace | Source: skills | Files: 2

### [cypress

- **cypress-testing** — >
  Scope: workspace | Source: skills | Files: 2

### [data-engineering

- **data-pipelines** — >
  Scope: workspace | Source: skills | Files: 2

### [data-quality

- **data-quality** — >
  Scope: workspace | Source: skills | Files: 2

### [data-science

- **data-science** — >
  Scope: workspace | Source: skills | Files: 3

### [data-warehouse

- **data-warehousing** — >
  Scope: workspace | Source: skills | Files: 2

### [database

- **database-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### [dbt

- **analytics-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### [debugging

- **debugging-tools** — >
  Scope: workspace | Source: skills | Files: 2

### [dependencies

- **live-dependency-resolver** — >
  Scope: workspace | Source: skills | Files: 2

### [developer-experience

- **developer-experience** — >
  Scope: workspace | Source: skills | Files: 2

### [devrel

- **developer-advocacy** — >
  Scope: workspace | Source: skills | Files: 2

### [docker

- **docker-kubernetes** — >
  Scope: workspace | Source: skills | Files: 2

### [documentation

- **codedocs** — >
  Scope: workspace | Source: skills | Files: 2

### [edge

- **edge-computing** — >
  Scope: workspace | Source: skills | Files: 2

### [elevenlabs

- **video-audio-design** — >
  Scope: workspace | Source: skills | Files: 2

### [email

- **email-deliverability** — >
  Scope: workspace | Source: skills | Files: 2

### [email-marketing

- **email-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### [employment-law

- **employment-law** — >
  Scope: workspace | Source: skills | Files: 2

### [engagement

- **employee-engagement** — >
  Scope: workspace | Source: skills | Files: 2

### [event-sourcing

- **event-driven-architecture** — >
  Scope: workspace | Source: skills | Files: 2

### [excel

- **spreadsheet-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### [ffmpeg

- **video-analyzer** — >
  Scope: workspace | Source: skills | Files: 2

### [figma

- **figma-to-code** — >
  Scope: workspace | Source: skills | Files: 2

### [financial-modeling

- **financial-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### [financial-reporting

- **financial-reporting** — >
  Scope: workspace | Source: skills | Files: 2

### [frontend

- **frontend-developer** — >
  Scope: workspace | Source: skills | Files: 2

### [fundraising

- **startup-fundraising** — >
  Scope: workspace | Source: skills | Files: 2

### [game-audio

- **game-audio** — >
  Scope: workspace | Source: skills | Files: 2

### [game-design

- **game-balancing** — >
  Scope: workspace | Source: skills | Files: 2

### [game-dev

- **game-design-patterns** — >
  Scope: workspace | Source: skills | Files: 2

### [gcp

- **cloud-gcp** — >
  Scope: workspace | Source: skills | Files: 2

### [git

- **git-advanced** — >
  Scope: workspace | Source: skills | Files: 2

### [github

- **address-pr-comments** — >
  Scope: workspace | Source: skills | Files: 2

### [growth

- **growth-hacking** — >
  Scope: workspace | Source: skills | Files: 2

### [i18n

- **localization-i18n** — >
  Scope: workspace | Source: skills | Files: 2

### [incident-response

- **security-incident-response** — >
  Scope: workspace | Source: skills | Files: 2

### [incidents

- **incident-management** — >
  Scope: workspace | Source: skills | Files: 2

### [interviewing

- **technical-interviewing** — >
  Scope: workspace | Source: skills | Files: 2

### [interviews

- **interview-design** — >
  Scope: workspace | Source: skills | Files: 2

### [ios

- **ios-swift** — >
  Scope: workspace | Source: skills | Files: 2

### [ip

- **ip-management** — >
  Scope: workspace | Source: skills | Files: 2

### [jest

- **jest-vitest** — >
  Scope: workspace | Source: skills | Files: 2

### [kafka

- **real-time-streaming** — >
  Scope: workspace | Source: skills | Files: 2

### [knowledge-base

- **knowledge-base** — >
  Scope: workspace | Source: skills | Files: 2

### [lead-scoring

- **lead-scoring** — >
  Scope: workspace | Source: skills | Files: 2

### [linux

- **linux-admin** — >
  Scope: workspace | Source: skills | Files: 2

### [llm

- **llm-app-development** — >
  Scope: workspace | Source: skills | Files: 2

### [load-testing

- **load-testing** — >
  Scope: workspace | Source: skills | Files: 2

### [marketing

- **absolute-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### [meta

- **meta-repo** — >
  Scope: workspace | Source: skills | Files: 3

### [microservices

- **microservices** — >
  Scope: workspace | Source: skills | Files: 2

### [mlops

- **ml-ops** — >
  Scope: workspace | Source: skills | Files: 2

### [mobile

- **mobile-testing** — >
  Scope: workspace | Source: skills | Files: 2

### [monitoring

- **supaguard** — >
  Scope: workspace | Source: skills | Files: 2

### [monorepo

- **monorepo-management** — >
  Scope: workspace | Source: skills | Files: 2

### [neovim

- **vim-neovim** — >
  Scope: workspace | Source: skills | Files: 2

### [nlp

- **nlp-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### [observability

- **observability** — >
  Scope: workspace | Source: skills | Files: 2

### [onboarding

- **onboarding** — >
  Scope: workspace | Source: skills | Files: 2

### [open-source

- **open-source-management** — >
  Scope: workspace | Source: skills | Files: 2

### [partnerships

- **partnership-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### [pentest

- **penetration-testing** — >
  Scope: workspace | Source: skills | Files: 2

### [performance

- **performance-engineering** — >
  Scope: workspace | Source: skills | Files: 2
- **performance-management** — >
  Scope: workspace | Source: skills | Files: 2

### [pixel-art

- **pixel-art-sprites** — >
  Scope: workspace | Source: skills | Files: 2

### [playwright

- **playwright-testing** — >
  Scope: workspace | Source: skills | Files: 3

### [presentation

- **presentation-design** — >
  Scope: workspace | Source: skills | Files: 3

### [pricing

- **pricing-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### [privacy

- **privacy-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### [product-analytics

- **product-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### [product-discovery

- **product-discovery** — >
  Scope: workspace | Source: skills | Files: 2

### [product-launch

- **product-launch** — >
  Scope: workspace | Source: skills | Files: 2

### [product-strategy

- **product-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### [project-management

- **project-execution** — >
  Scope: workspace | Source: skills | Files: 2

### [prompts

- **prompt-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### [proposals

- **proposal-writing** — >
  Scope: workspace | Source: skills | Files: 2

### [react

- **react-ink** — >
  Scope: workspace | Source: skills | Files: 3

### [react-native

- **react-native** — >
  Scope: workspace | Source: skills | Files: 2

### [recruiting

- **recruiting-ops** — >
  Scope: workspace | Source: skills | Files: 2

### [refactoring

- **refactoring-patterns** — >
  Scope: workspace | Source: skills | Files: 2

### [regex

- **regex-mastery** — >
  Scope: workspace | Source: skills | Files: 2

### [remote-work

- **remote-collaboration** — >
  Scope: workspace | Source: skills | Files: 2

### [remotion

- **remotion-video** — >
  Scope: workspace | Source: skills | Files: 2

### [rfc

- **internal-docs** — >
  Scope: workspace | Source: skills | Files: 2

### [saas

- **saas-metrics** — >
  Scope: workspace | Source: skills | Files: 2

### [sales

- **sales-playbook** — >
  Scope: workspace | Source: skills | Files: 2

### [sales-enablement

- **sales-enablement** — >
  Scope: workspace | Source: skills | Files: 2

### [scriptwriting

- **video-scriptwriting** — >
  Scope: workspace | Source: skills | Files: 2

### [second-brain

- **second-brain** — >
  Scope: workspace | Source: skills | Files: 2

### [security

- **appsec-owasp** — >
  Scope: workspace | Source: skills | Files: 2
- **skill-audit** — >
  Scope: workspace | Source: skills | Files: 2

### [sentry

- **sentry** — >
  Scope: workspace | Source: skills | Files: 3

### [seo

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2
- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### [signoz

- **signoz** — >
  Scope: workspace | Source: skills | Files: 3

### [simplification

- **absolute-simplify** — >
  Scope: workspace | Source: skills | Files: 2

### [skill-creation

- **skill-forge** — >
  Scope: workspace | Source: skills | Files: 1

### [social-media

- **social-media-scheduling** — >
  Scope: workspace | Source: skills | Files: 2
- **social-media-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### [solidity

- **web3-smart-contracts** — >
  Scope: workspace | Source: skills | Files: 2

### [sre

- **site-reliability** — >
  Scope: workspace | Source: skills | Files: 2

### [support

- **customer-support-ops** — >
  Scope: workspace | Source: skills | Files: 2

### [support-analytics

- **support-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### [tax

- **tax-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### [technical-writing

- **technical-writing** — >
  Scope: workspace | Source: skills | Files: 2

### [terminal

- **cmux** — >
  Scope: workspace | Source: skills | Files: 3

### [terraform

- **terraform-iac** — >
  Scope: workspace | Source: skills | Files: 2

### [testing

- **test-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### [ui

- **absolute-ui** — >
  Scope: workspace | Source: skills | Files: 3

### [unity

- **unity-development** — >
  Scope: workspace | Source: skills | Files: 2

### [user-stories

- **user-stories** — >
  Scope: workspace | Source: skills | Files: 2

### [ux-research

- **ux-research** — >
  Scope: workspace | Source: skills | Files: 2

### [video-creation

- **video-creator** — >
  Scope: workspace | Source: skills | Files: 2

### [video-production

- **video-production** — >
  Scope: workspace | Source: skills | Files: 2

### [vite

- **vite-plus** — >
  Scope: workspace | Source: skills | Files: 3

### [x-api

- **x-twitter-scraper** — >
  Scope: workspace | Source: skills | Files: 2

### [zapier

- **no-code-automation** — >
  Scope: workspace | Source: skills | Files: 2

### a11y

- **accessibility-wcag** — >
  Scope: workspace | Source: skills | Files: 2

### a2a-protocol

- **a2ui** — >
  Scope: workspace | Source: skills | Files: 3

### ab-testing

- **posthog** — >
  Scope: workspace | Source: skills | Files: 3

### acceptance-criteria

- **user-stories** — >
  Scope: workspace | Source: skills | Files: 2

### accessibility

- **frontend-developer** — >
  Scope: workspace | Source: skills | Files: 2

### accessibility]

- **absolute-ui** — >
  Scope: workspace | Source: skills | Files: 3

### activation

- **growth-hacking** — >
  Scope: workspace | Source: skills | Files: 2

### adaptive-music

- **game-audio** — >
  Scope: workspace | Source: skills | Files: 2

### adoption

- **product-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### adr

- **technical-writing** — >
  Scope: workspace | Source: skills | Files: 2

### advocacy

- **community-management** — >
  Scope: workspace | Source: skills | Files: 2

### aeo

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2
- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### affiliates

- **partnership-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### agent-card

- **a2a-protocol** — >
  Scope: workspace | Source: skills | Files: 3

### agent-definitions]

- **skill-audit** — >
  Scope: workspace | Source: skills | Files: 2
- **skill-forge** — >
  Scope: workspace | Source: skills | Files: 1

### agent-interfaces]

- **a2ui** — >
  Scope: workspace | Source: skills | Files: 3

### agent-interoperability

- **a2a-protocol** — >
  Scope: workspace | Source: skills | Files: 3

### agent-safety

- **skill-audit** — >
  Scope: workspace | Source: skills | Files: 2

### agent-skills

- **skill-forge** — >
  Scope: workspace | Source: skills | Files: 1

### agent-ui

- **a2ui** — >
  Scope: workspace | Source: skills | Files: 3

### ai-agent

- **codedocs** — >
  Scope: workspace | Source: skills | Files: 2

### ai-apps

- **llm-app-development** — >
  Scope: workspace | Source: skills | Files: 2

### ai-search

- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### ai-vision

- **video-analyzer** — >
  Scope: workspace | Source: skills | Files: 2

### ai]

- **prompt-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### airflow

- **data-pipelines** — >
  Scope: workspace | Source: skills | Files: 2

### alerting

- **observability** — >
  Scope: workspace | Source: skills | Files: 2

### analysis]

- **data-science** — >
  Scope: workspace | Source: skills | Files: 3
- **financial-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### analytics

- **analytics-engineering** — >
  Scope: workspace | Source: skills | Files: 2
- **social-media-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### android

- **react-native** — >
  Scope: workspace | Source: skills | Files: 2

### animation

- **pixel-art-sprites** — >
  Scope: workspace | Source: skills | Files: 2
- **remotion-video** — >
  Scope: workspace | Source: skills | Files: 2

### ap-ar

- **bookkeeping-automation** — >
  Scope: workspace | Source: skills | Files: 2

### api

- **x-twitter-scraper** — >
  Scope: workspace | Source: skills | Files: 2

### api-design

- **backend-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### api-docs

- **technical-writing** — >
  Scope: workspace | Source: skills | Files: 2

### appium

- **mobile-testing** — >
  Scope: workspace | Source: skills | Files: 2

### appsec]

- **appsec-owasp** — >
  Scope: workspace | Source: skills | Files: 2

### architecture

- **absolute-brainstorm** — >
  Scope: workspace | Source: skills | Files: 2
- **clean-architecture** — >
  Scope: workspace | Source: skills | Files: 2
- **codedocs** — >
  Scope: workspace | Source: skills | Files: 2
- **event-driven-architecture** — >
  Scope: workspace | Source: skills | Files: 2
- **microservices** — >
  Scope: workspace | Source: skills | Files: 2

### argument-parsing

- **cli-design** — >
  Scope: workspace | Source: skills | Files: 2

### aria

- **accessibility-wcag** — >
  Scope: workspace | Source: skills | Files: 2

### articles

- **knowledge-base** — >
  Scope: workspace | Source: skills | Files: 2

### assessment]

- **interview-design** — >
  Scope: workspace | Source: skills | Files: 2

### assumptions

- **product-discovery** — >
  Scope: workspace | Source: skills | Files: 2

### async

- **remote-collaboration** — >
  Scope: workspace | Source: skills | Files: 2

### audio-design

- **video-audio-design** — >
  Scope: workspace | Source: skills | Files: 2

### audio-mixing]

- **video-audio-design** — >
  Scope: workspace | Source: skills | Files: 2

### audit

- **regulatory-compliance** — >
  Scope: workspace | Source: skills | Files: 2
- **skill-audit** — >
  Scope: workspace | Source: skills | Files: 2

### audit]

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2
- **penetration-testing** — >
  Scope: workspace | Source: skills | Files: 2

### authentication

- **appsec-owasp** — >
  Scope: workspace | Source: skills | Files: 2

### automated-review

- **code-review-mastery** — >
  Scope: workspace | Source: skills | Files: 2

### automation

- **ci-cd-pipelines** — >
  Scope: workspace | Source: skills | Files: 2
- **cmux** — >
  Scope: workspace | Source: skills | Files: 3
- **no-code-automation** — >
  Scope: workspace | Source: skills | Files: 2
- **x-twitter-scraper** — >
  Scope: workspace | Source: skills | Files: 2

### automation]

- **crm-management** — >
  Scope: workspace | Source: skills | Files: 2
- **cypress-testing** — >
  Scope: workspace | Source: skills | Files: 2
- **email-marketing** — >
  Scope: workspace | Source: skills | Files: 2
- **shell-scripting** — >
  Scope: workspace | Source: skills | Files: 2

### autonomous

- **absolute-simplify** — >
  Scope: workspace | Source: skills | Files: 2

### background-music

- **video-audio-design** — >
  Scope: workspace | Source: skills | Files: 2

### backlog

- **user-stories** — >
  Scope: workspace | Source: skills | Files: 2

### balance-sheet

- **financial-reporting** — >
  Scope: workspace | Source: skills | Files: 2

### balancing

- **game-balancing** — >
  Scope: workspace | Source: skills | Files: 2

### battle-cards

- **sales-enablement** — >
  Scope: workspace | Source: skills | Files: 2

### behavioral-analytics]

- **customer-research** — >
  Scope: workspace | Source: skills | Files: 2

### benchmarking

- **load-testing** — >
  Scope: workspace | Source: skills | Files: 2
- **performance-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### benefits]

- **compensation-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### best-practices]

- **clean-code** — >
  Scope: workspace | Source: skills | Files: 2

### beta

- **product-launch** — >
  Scope: workspace | Source: skills | Files: 2

### bigquery

- **cloud-gcp** — >
  Scope: workspace | Source: skills | Files: 2
- **data-warehousing** — >
  Scope: workspace | Source: skills | Files: 2

### billing]

- **api-monetization** — >
  Scope: workspace | Source: skills | Files: 3

### bisect

- **git-advanced** — >
  Scope: workspace | Source: skills | Files: 2

### blog

- **content-marketing** — >
  Scope: workspace | Source: skills | Files: 2
- **developer-advocacy** — >
  Scope: workspace | Source: skills | Files: 2

### board-decks]

- **financial-reporting** — >
  Scope: workspace | Source: skills | Files: 2

### board-reporting]

- **saas-metrics** — >
  Scope: workspace | Source: skills | Files: 2

### boundaries

- **clean-architecture** — >
  Scope: workspace | Source: skills | Files: 2

### brand]

- **absolute-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### breach

- **security-incident-response** — >
  Scope: workspace | Source: skills | Files: 2

### browser-automation

- **playwright-testing** — >
  Scope: workspace | Source: skills | Files: 3

### buddy-system

- **onboarding** — >
  Scope: workspace | Source: skills | Files: 2

### build-caching

- **monorepo-management** — >
  Scope: workspace | Source: skills | Files: 2

### bundler

- **vite-plus** — >
  Scope: workspace | Source: skills | Files: 3

### business-writing]

- **proposal-writing** — >
  Scope: workspace | Source: skills | Files: 2

### cac

- **saas-metrics** — >
  Scope: workspace | Source: skills | Files: 2

### calibration

- **performance-management** — >
  Scope: workspace | Source: skills | Files: 2

### campaign-planning

- **social-media-scheduling** — >
  Scope: workspace | Source: skills | Files: 2

### campaigns

- **email-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### cap-table

- **financial-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### capacity

- **site-reliability** — >
  Scope: workspace | Source: skills | Files: 2

### capacity]

- **load-testing** — >
  Scope: workspace | Source: skills | Files: 2

### career-ladders

- **performance-management** — >
  Scope: workspace | Source: skills | Files: 2

### cargo

- **live-dependency-resolver** — >
  Scope: workspace | Source: skills | Files: 2

### case-studies

- **sales-enablement** — >
  Scope: workspace | Source: skills | Files: 2

### cash-flow

- **financial-reporting** — >
  Scope: workspace | Source: skills | Files: 2

### ccpa

- **privacy-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### cdc

- **real-time-streaming** — >
  Scope: workspace | Source: skills | Files: 2

### cdn

- **edge-computing** — >
  Scope: workspace | Source: skills | Files: 2

### chain-of-thought

- **prompt-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### changelog

- **developer-experience** — >
  Scope: workspace | Source: skills | Files: 2
- **open-source-management** — >
  Scope: workspace | Source: skills | Files: 2

### channel

- **partnership-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### chart-of-accounts]

- **bookkeeping-automation** — >
  Scope: workspace | Source: skills | Files: 2

### churn

- **customer-success-playbook** — >
  Scope: workspace | Source: skills | Files: 2
- **saas-metrics** — >
  Scope: workspace | Source: skills | Files: 2

### citation-score]

- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### clarity]

- **absolute-simplify** — >
  Scope: workspace | Source: skills | Files: 2

### classification]

- **nlp-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### clean-code

- **absolute-simplify** — >
  Scope: workspace | Source: skills | Files: 2
- **refactoring-patterns** — >
  Scope: workspace | Source: skills | Files: 2

### cli

- **react-ink** — >
  Scope: workspace | Source: skills | Files: 3
- **shell-scripting** — >
  Scope: workspace | Source: skills | Files: 2

### cli]

- **cmux** — >
  Scope: workspace | Source: skills | Files: 3

### closing]

- **sales-playbook** — >
  Scope: workspace | Source: skills | Files: 2

### cloud

- **cloud-aws** — >
  Scope: workspace | Source: skills | Files: 2

### cloud-run

- **cloud-gcp** — >
  Scope: workspace | Source: skills | Files: 2

### cloudflare-workers

- **edge-computing** — >
  Scope: workspace | Source: skills | Files: 2

### cnn]

- **computer-vision** — >
  Scope: workspace | Source: skills | Files: 2

### co-marketing

- **partnership-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### code-generation

- **skill-forge** — >
  Scope: workspace | Source: skills | Files: 1

### code-quality

- **absolute-simplify** — >
  Scope: workspace | Source: skills | Files: 2
- **refactoring-patterns** — >
  Scope: workspace | Source: skills | Files: 2

### code-review

- **address-pr-comments** — >
  Scope: workspace | Source: skills | Files: 2
- **clean-code** — >
  Scope: workspace | Source: skills | Files: 2

### code-understanding]

- **codedocs** — >
  Scope: workspace | Source: skills | Files: 2

### codebase

- **codedocs** — >
  Scope: workspace | Source: skills | Files: 2

### coding-challenges

- **interview-design** — >
  Scope: workspace | Source: skills | Files: 2
- **technical-interviewing** — >
  Scope: workspace | Source: skills | Files: 2

### cohort-analysis

- **saas-metrics** — >
  Scope: workspace | Source: skills | Files: 2

### cohorts

- **product-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### collaboration

- **remote-collaboration** — >
  Scope: workspace | Source: skills | Files: 2

### command-pattern]

- **game-design-patterns** — >
  Scope: workspace | Source: skills | Files: 2

### community

- **developer-advocacy** — >
  Scope: workspace | Source: skills | Files: 2
- **open-source-management** — >
  Scope: workspace | Source: skills | Files: 2

### community]

- **social-media-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### competitive-intel

- **sales-enablement** — >
  Scope: workspace | Source: skills | Files: 2

### competitor-analysis

- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### compliance

- **cloud-security** — >
  Scope: workspace | Source: skills | Files: 2
- **tax-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### component-testing

- **cypress-testing** — >
  Scope: workspace | Source: skills | Files: 2

### components

- **react-ink** — >
  Scope: workspace | Source: skills | Files: 3

### components]

- **clean-architecture** — >
  Scope: workspace | Source: skills | Files: 2

### config

- **cli-design** — >
  Scope: workspace | Source: skills | Files: 2

### consent

- **privacy-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### containers

- **docker-kubernetes** — >
  Scope: workspace | Source: skills | Files: 2

### containment]

- **security-incident-response** — >
  Scope: workspace | Source: skills | Files: 2

### content

- **social-media-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### content-calendar

- **social-media-scheduling** — >
  Scope: workspace | Source: skills | Files: 2

### content-planning

- **video-scriptwriting** — >
  Scope: workspace | Source: skills | Files: 2

### content-strategy

- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### content-strategy]

- **content-marketing** — >
  Scope: workspace | Source: skills | Files: 2
- **social-media-scheduling** — >
  Scope: workspace | Source: skills | Files: 2

### context

- **second-brain** — >
  Scope: workspace | Source: skills | Files: 2

### contract-testing

- **api-testing** — >
  Scope: workspace | Source: skills | Files: 2

### contractor

- **employment-law** — >
  Scope: workspace | Source: skills | Files: 2

### contracts

- **data-quality** — >
  Scope: workspace | Source: skills | Files: 2

### controls]

- **regulatory-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### copy]

- **copywriting** — >
  Scope: workspace | Source: skills | Files: 2

### copyright]

- **ip-management** — >
  Scope: workspace | Source: skills | Files: 2

### copywriting

- **absolute-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### core-data

- **ios-swift** — >
  Scope: workspace | Source: skills | Files: 2

### core-web-vitals

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2

### coroutines

- **android-kotlin** — >
  Scope: workspace | Source: skills | Files: 2

### corporate-tax]

- **tax-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### cost-allocation]

- **budgeting-planning** — >
  Scope: workspace | Source: skills | Files: 2

### coverage

- **jest-vitest** — >
  Scope: workspace | Source: skills | Files: 2
- **test-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### cqrs

- **event-driven-architecture** — >
  Scope: workspace | Source: skills | Files: 2

### crash-reporting]

- **mobile-testing** — >
  Scope: workspace | Source: skills | Files: 2

### cro

- **absolute-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### cross-platform]

- **react-native** — >
  Scope: workspace | Source: skills | Files: 2

### csat

- **support-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### csharp

- **unity-development** — >
  Scope: workspace | Source: skills | Files: 2

### css

- **absolute-ui** — >
  Scope: workspace | Source: skills | Files: 3
- **frontend-developer** — >
  Scope: workspace | Source: skills | Files: 2

### css]

- **figma-to-code** — >
  Scope: workspace | Source: skills | Files: 2

### cta

- **copywriting** — >
  Scope: workspace | Source: skills | Files: 2

### culture

- **employee-engagement** — >
  Scope: workspace | Source: skills | Files: 2

### dashboards

- **spreadsheet-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### data-protection]

- **privacy-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### data-visualization

- **presentation-design** — >
  Scope: workspace | Source: skills | Files: 3

### data-warehouse

- **analytics-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### data]

- **cloud-gcp** — >
  Scope: workspace | Source: skills | Files: 2

### databases

- **backend-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### dbt

- **data-pipelines** — >
  Scope: workspace | Source: skills | Files: 2

### dcf

- **financial-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### debugging]

- **sentry** — >
  Scope: workspace | Source: skills | Files: 3

### declarative-ui

- **a2ui** — >
  Scope: workspace | Source: skills | Files: 3

### deep-learning

- **computer-vision** — >
  Scope: workspace | Source: skills | Files: 2

### defi

- **web3-smart-contracts** — >
  Scope: workspace | Source: skills | Files: 2

### deflection

- **support-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### deliverability

- **email-deliverability** — >
  Scope: workspace | Source: skills | Files: 2
- **email-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### demos

- **developer-advocacy** — >
  Scope: workspace | Source: skills | Files: 2

### dependencies

- **project-execution** — >
  Scope: workspace | Source: skills | Files: 2

### dependency-rule

- **clean-architecture** — >
  Scope: workspace | Source: skills | Files: 2

### deployment

- **ci-cd-pipelines** — >
  Scope: workspace | Source: skills | Files: 2

### design

- **absolute-brainstorm** — >
  Scope: workspace | Source: skills | Files: 2
- **absolute-ui** — >
  Scope: workspace | Source: skills | Files: 3

### design-docs

- **internal-docs** — >
  Scope: workspace | Source: skills | Files: 2

### design-handoff

- **figma-to-code** — >
  Scope: workspace | Source: skills | Files: 2

### design-patterns

- **game-design-patterns** — >
  Scope: workspace | Source: skills | Files: 2

### design-system]

- **video-analyzer** — >
  Scope: workspace | Source: skills | Files: 2

### design]

- **api-design** — >
  Scope: workspace | Source: skills | Files: 2
- **system-design** — >
  Scope: workspace | Source: skills | Files: 2

### detox

- **mobile-testing** — >
  Scope: workspace | Source: skills | Files: 2

### device-farm

- **mobile-testing** — >
  Scope: workspace | Source: skills | Files: 2

### devops]

- **ci-cd-pipelines** — >
  Scope: workspace | Source: skills | Files: 2
- **docker-kubernetes** — >
  Scope: workspace | Source: skills | Files: 2
- **terraform-iac** — >
  Scope: workspace | Source: skills | Files: 2

### devtools

- **debugging-tools** — >
  Scope: workspace | Source: skills | Files: 2

### difficulty]

- **game-balancing** — >
  Scope: workspace | Source: skills | Files: 2

### dimensional-modeling

- **data-warehousing** — >
  Scope: workspace | Source: skills | Files: 2

### discovery

- **sales-playbook** — >
  Scope: workspace | Source: skills | Files: 2

### distributed-systems

- **backend-engineering** — >
  Scope: workspace | Source: skills | Files: 2
- **microservices** — >
  Scope: workspace | Source: skills | Files: 2
- **system-design** — >
  Scope: workspace | Source: skills | Files: 2

### distributed-systems]

- **event-driven-architecture** — >
  Scope: workspace | Source: skills | Files: 2

### distributed-teams]

- **remote-collaboration** — >
  Scope: workspace | Source: skills | Files: 2

### distribution

- **cli-design** — >
  Scope: workspace | Source: skills | Files: 2

### dkim

- **email-deliverability** — >
  Scope: workspace | Source: skills | Files: 2

### dmarc

- **email-deliverability** — >
  Scope: workspace | Source: skills | Files: 2

### documentation

- **remote-collaboration** — >
  Scope: workspace | Source: skills | Files: 2
- **technical-writing** — >
  Scope: workspace | Source: skills | Files: 2

### documentation]

- **internal-docs** — >
  Scope: workspace | Source: skills | Files: 2
- **knowledge-base** — >
  Scope: workspace | Source: skills | Files: 2

### dpia

- **privacy-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### drip-sequences

- **email-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### dx]

- **developer-experience** — >
  Scope: workspace | Source: skills | Files: 2

### e2e

- **cypress-testing** — >
  Scope: workspace | Source: skills | Files: 2
- **playwright-testing** — >
  Scope: workspace | Source: skills | Files: 3

### ecommerce-seo

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2

### economy

- **game-balancing** — >
  Scope: workspace | Source: skills | Files: 2

### ecosystem]

- **partnership-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### ecs

- **unity-development** — >
  Scope: workspace | Source: skills | Files: 2

### eda

- **data-science** — >
  Scope: workspace | Source: skills | Files: 3

### edge-functions]

- **edge-computing** — >
  Scope: workspace | Source: skills | Files: 2

### editing]

- **video-production** — >
  Scope: workspace | Source: skills | Files: 2

### editor

- **vim-neovim** — >
  Scope: workspace | Source: skills | Files: 2

### editorial

- **content-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### embeddings

- **nlp-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### encryption

- **cryptography** — >
  Scope: workspace | Source: skills | Files: 2

### engagement

- **community-management** — >
  Scope: workspace | Source: skills | Files: 2
- **social-media-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### enps

- **employee-engagement** — >
  Scope: workspace | Source: skills | Files: 2

### equity

- **compensation-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### error-budgets

- **site-reliability** — >
  Scope: workspace | Source: skills | Files: 2

### error-monitoring

- **sentry** — >
  Scope: workspace | Source: skills | Files: 3

### error-tracking

- **posthog** — >
  Scope: workspace | Source: skills | Files: 3

### escalation

- **customer-support-ops** — >
  Scope: workspace | Source: skills | Files: 2

### estimation

- **agile-scrum** — >
  Scope: workspace | Source: skills | Files: 2

### estimation]

- **user-stories** — >
  Scope: workspace | Source: skills | Files: 2

### ethical-hacking

- **penetration-testing** — >
  Scope: workspace | Source: skills | Files: 2

### etl

- **data-pipelines** — >
  Scope: workspace | Source: skills | Files: 2

### evaluation

- **llm-app-development** — >
  Scope: workspace | Source: skills | Files: 2

### event-sourcing

- **real-time-streaming** — >
  Scope: workspace | Source: skills | Files: 2

### event-system

- **game-design-patterns** — >
  Scope: workspace | Source: skills | Files: 2

### eventual-consistency

- **event-driven-architecture** — >
  Scope: workspace | Source: skills | Files: 2

### execution

- **project-execution** — >
  Scope: workspace | Source: skills | Files: 2

### expansion

- **account-management** — >
  Scope: workspace | Source: skills | Files: 2
- **customer-success-playbook** — >
  Scope: workspace | Source: skills | Files: 2

### expo

- **react-native** — >
  Scope: workspace | Source: skills | Files: 2

### fault-injection

- **chaos-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### feature-flags

- **posthog** — >
  Scope: workspace | Source: skills | Files: 3

### feature-store

- **ml-ops** — >
  Scope: workspace | Source: skills | Files: 2

### feedback

- **community-management** — >
  Scope: workspace | Source: skills | Files: 2

### few-shot

- **prompt-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### fine-tuning

- **llm-app-development** — >
  Scope: workspace | Source: skills | Files: 2

### flaky-tests]

- **playwright-testing** — >
  Scope: workspace | Source: skills | Files: 3

### flink

- **real-time-streaming** — >
  Scope: workspace | Source: skills | Files: 2

### fmod

- **game-audio** — >
  Scope: workspace | Source: skills | Files: 2

### forecasting

- **budgeting-planning** — >
  Scope: workspace | Source: skills | Files: 2
- **crm-management** — >
  Scope: workspace | Source: skills | Files: 2
- **financial-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### forensics

- **security-incident-response** — >
  Scope: workspace | Source: skills | Files: 2

### formulas

- **spreadsheet-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### forums]

- **community-management** — >
  Scope: workspace | Source: skills | Files: 2

### fpa

- **budgeting-planning** — >
  Scope: workspace | Source: skills | Files: 2

### frame-extraction

- **video-analyzer** — >
  Scope: workspace | Source: skills | Files: 2

### freemium

- **pricing-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### funnels

- **product-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### game-days]

- **chaos-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### gamedev

- **unity-development** — >
  Scope: workspace | Source: skills | Files: 2

### gamedev]

- **pixel-art-sprites** — >
  Scope: workspace | Source: skills | Files: 2

### gas-optimization]

- **web3-smart-contracts** — >
  Scope: workspace | Source: skills | Files: 2

### gdpr

- **privacy-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### gem

- **live-dependency-resolver** — >
  Scope: workspace | Source: skills | Files: 2

### geo

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2
- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### gh-cli

- **address-pr-comments** — >
  Scope: workspace | Source: skills | Files: 2

### git

- **meta-repo** — >
  Scope: workspace | Source: skills | Files: 3

### git-diff

- **code-review-mastery** — >
  Scope: workspace | Source: skills | Files: 2

### github-actions

- **ci-cd-pipelines** — >
  Scope: workspace | Source: skills | Files: 2

### go-modules

- **live-dependency-resolver** — >
  Scope: workspace | Source: skills | Files: 2

### go-to-market]

- **product-launch** — >
  Scope: workspace | Source: skills | Files: 2

### google-adk

- **a2ui** — >
  Scope: workspace | Source: skills | Files: 3

### google-cloud

- **cloud-gcp** — >
  Scope: workspace | Source: skills | Files: 2

### google-sheets

- **spreadsheet-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### governance

- **open-source-management** — >
  Scope: workspace | Source: skills | Files: 2

### graphql

- **api-design** — >
  Scope: workspace | Source: skills | Files: 2

### graphql]

- **api-testing** — >
  Scope: workspace | Source: skills | Files: 2

### great-expectations

- **data-quality** — >
  Scope: workspace | Source: skills | Files: 2

### growth

- **absolute-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### growth]

- **pricing-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### grpc

- **api-design** — >
  Scope: workspace | Source: skills | Files: 2

### grpc]

- **a2a-protocol** — >
  Scope: workspace | Source: skills | Files: 3

### gtm

- **product-launch** — >
  Scope: workspace | Source: skills | Files: 2

### guardrails

- **llm-app-development** — >
  Scope: workspace | Source: skills | Files: 2

### hashing

- **cryptography** — >
  Scope: workspace | Source: skills | Files: 2

### hcl

- **terraform-iac** — >
  Scope: workspace | Source: skills | Files: 2

### headlines

- **copywriting** — >
  Scope: workspace | Source: skills | Files: 2

### health-checks]

- **supaguard** — >
  Scope: workspace | Source: skills | Files: 2

### health-scores

- **customer-success-playbook** — >
  Scope: workspace | Source: skills | Files: 2

### helm

- **docker-kubernetes** — >
  Scope: workspace | Source: skills | Files: 2

### help-center

- **knowledge-base** — >
  Scope: workspace | Source: skills | Files: 2

### hipaa

- **regulatory-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### hiring

- **interview-design** — >
  Scope: workspace | Source: skills | Files: 2
- **recruiting-ops** — >
  Scope: workspace | Source: skills | Files: 2
- **technical-interviewing** — >
  Scope: workspace | Source: skills | Files: 2

### hooks

- **git-advanced** — >
  Scope: workspace | Source: skills | Files: 2

### hubspot

- **crm-management** — >
  Scope: workspace | Source: skills | Files: 2

### iac

- **terraform-iac** — >
  Scope: workspace | Source: skills | Files: 2

### iam

- **cloud-security** — >
  Scope: workspace | Source: skills | Files: 2

### icp

- **lead-scoring** — >
  Scope: workspace | Source: skills | Files: 2

### icu-message-format]

- **localization-i18n** — >
  Scope: workspace | Source: skills | Files: 2

### identity]

- **brand-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### implementation

- **figma-to-code** — >
  Scope: workspace | Source: skills | Files: 2

### incident-management]

- **site-reliability** — >
  Scope: workspace | Source: skills | Files: 2

### indexing

- **database-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### infrastructure

- **cloud-aws** — >
  Scope: workspace | Source: skills | Files: 2
- **system-design** — >
  Scope: workspace | Source: skills | Files: 2
- **terraform-iac** — >
  Scope: workspace | Source: skills | Files: 2

### integrations

- **partnership-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### intent-signals

- **lead-scoring** — >
  Scope: workspace | Source: skills | Files: 2

### interactive]

- **react-ink** — >
  Scope: workspace | Source: skills | Files: 3

### internal-tooling]

- **no-code-automation** — >
  Scope: workspace | Source: skills | Files: 2

### international-seo

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2

### interviewing]

- **absolute-brainstorm** — >
  Scope: workspace | Source: skills | Files: 2

### interviews

- **customer-research** — >
  Scope: workspace | Source: skills | Files: 2

### investors

- **startup-fundraising** — >
  Scope: workspace | Source: skills | Files: 2

### ios

- **react-native** — >
  Scope: workspace | Source: skills | Files: 2

### javascript

- **frontend-developer** — >
  Scope: workspace | Source: skills | Files: 2

### jetpack-compose

- **android-kotlin** — >
  Scope: workspace | Source: skills | Files: 2

### journey-maps

- **ux-research** — >
  Scope: workspace | Source: skills | Files: 2

### json-rpc

- **a2a-protocol** — >
  Scope: workspace | Source: skills | Files: 3

### jtbd

- **product-discovery** — >
  Scope: workspace | Source: skills | Files: 2

### jwt

- **cryptography** — >
  Scope: workspace | Source: skills | Files: 2

### k6

- **load-testing** — >
  Scope: workspace | Source: skills | Files: 2

### kanban

- **agile-scrum** — >
  Scope: workspace | Source: skills | Files: 2

### key-management]

- **cryptography** — >
  Scope: workspace | Source: skills | Files: 2

### keybindings]

- **vim-neovim** — >
  Scope: workspace | Source: skills | Files: 2

### keyboard

- **accessibility-wcag** — >
  Scope: workspace | Source: skills | Files: 2

### keywords

- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### knowledge-base

- **second-brain** — >
  Scope: workspace | Source: skills | Files: 2

### knowledge-management

- **internal-docs** — >
  Scope: workspace | Source: skills | Files: 2

### knowledge-transfer]

- **onboarding** — >
  Scope: workspace | Source: skills | Files: 2

### kotlin

- **android-kotlin** — >
  Scope: workspace | Source: skills | Files: 2

### kubernetes

- **docker-kubernetes** — >
  Scope: workspace | Source: skills | Files: 2

### l10n

- **localization-i18n** — >
  Scope: workspace | Source: skills | Files: 2

### landing-pages

- **copywriting** — >
  Scope: workspace | Source: skills | Files: 2

### latency

- **edge-computing** — >
  Scope: workspace | Source: skills | Files: 2
- **performance-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### launch-checklist

- **product-launch** — >
  Scope: workspace | Source: skills | Files: 2

### legal]

- **contract-drafting** — >
  Scope: workspace | Source: skills | Files: 2

### leveling

- **compensation-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### licensing

- **contract-drafting** — >
  Scope: workspace | Source: skills | Files: 2
- **ip-management** — >
  Scope: workspace | Source: skills | Files: 2
- **open-source-management** — >
  Scope: workspace | Source: skills | Files: 2

### lineage

- **data-quality** — >
  Scope: workspace | Source: skills | Files: 2

### link-building

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2

### linting

- **vite-plus** — >
  Scope: workspace | Source: skills | Files: 3

### llm

- **prompt-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### llm]

- **mastra** — >
  Scope: workspace | Source: skills | Files: 3

### local-review

- **code-review-mastery** — >
  Scope: workspace | Source: skills | Files: 2

### local-seo

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2

### localization

- **localization-i18n** — >
  Scope: workspace | Source: skills | Files: 2

### logging

- **observability** — >
  Scope: workspace | Source: skills | Files: 2

### logs

- **signoz** — >
  Scope: workspace | Source: skills | Files: 3

### ltv

- **saas-metrics** — >
  Scope: workspace | Source: skills | Files: 2

### lua

- **vim-neovim** — >
  Scope: workspace | Source: skills | Files: 2

### macros

- **customer-support-ops** — >
  Scope: workspace | Source: skills | Files: 2

### macros]

- **spreadsheet-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### maintainer]

- **open-source-management** — >
  Scope: workspace | Source: skills | Files: 2

### make

- **no-code-automation** — >
  Scope: workspace | Source: skills | Files: 2

### market-landscape

- **competitive-analysis** — >
  Scope: workspace | Source: skills | Files: 2

### matching]

- **regex-mastery** — >
  Scope: workspace | Source: skills | Files: 2

### mcp

- **mastra** — >
  Scope: workspace | Source: skills | Files: 3
- **x-twitter-scraper** — >
  Scope: workspace | Source: skills | Files: 2

### meddic

- **sales-playbook** — >
  Scope: workspace | Source: skills | Files: 2

### meetings

- **remote-collaboration** — >
  Scope: workspace | Source: skills | Files: 2

### memory

- **ai-agent-design** — >
  Scope: workspace | Source: skills | Files: 2
- **second-brain** — >
  Scope: workspace | Source: skills | Files: 2

### memory]

- **debugging-tools** — >
  Scope: workspace | Source: skills | Files: 2
- **performance-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### message-brokers

- **event-driven-architecture** — >
  Scope: workspace | Source: skills | Files: 2

### messaging

- **brand-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### metrics

- **analytics-engineering** — >
  Scope: workspace | Source: skills | Files: 2
- **observability** — >
  Scope: workspace | Source: skills | Files: 2
- **product-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### metrics]

- **signoz** — >
  Scope: workspace | Source: skills | Files: 3
- **support-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### migration

- **developer-experience** — >
  Scope: workspace | Source: skills | Files: 2

### migrations]

- **database-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### ml-pipelines]

- **ml-ops** — >
  Scope: workspace | Source: skills | Files: 2

### mobile

- **react-native** — >
  Scope: workspace | Source: skills | Files: 2

### mobile]

- **ios-swift** — >
  Scope: workspace | Source: skills | Files: 2

### mock-servers

- **api-testing** — >
  Scope: workspace | Source: skills | Files: 2

### mocking

- **jest-vitest** — >
  Scope: workspace | Source: skills | Files: 2

### model-deployment

- **ml-ops** — >
  Scope: workspace | Source: skills | Files: 2

### moderation

- **community-management** — >
  Scope: workspace | Source: skills | Files: 2

### modules

- **terraform-iac** — >
  Scope: workspace | Source: skills | Files: 2

### monetization

- **api-monetization** — >
  Scope: workspace | Source: skills | Files: 3
- **pricing-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### monitoring

- **ml-ops** — >
  Scope: workspace | Source: skills | Files: 2

### monitoring]

- **data-quality** — >
  Scope: workspace | Source: skills | Files: 2

### monorepo-migration

- **meta-repo** — >
  Scope: workspace | Source: skills | Files: 3

### monorepo]

- **vite-plus** — >
  Scope: workspace | Source: skills | Files: 3

### month-end

- **bookkeeping-automation** — >
  Scope: workspace | Source: skills | Files: 2

### mql

- **lead-scoring** — >
  Scope: workspace | Source: skills | Files: 2

### mrr

- **saas-metrics** — >
  Scope: workspace | Source: skills | Files: 2

### msa

- **contract-drafting** — >
  Scope: workspace | Source: skills | Files: 2

### multi-agent

- **a2a-protocol** — >
  Scope: workspace | Source: skills | Files: 3
- **ai-agent-design** — >
  Scope: workspace | Source: skills | Files: 2

### multi-repo

- **meta-repo** — >
  Scope: workspace | Source: skills | Files: 3

### n8n

- **no-code-automation** — >
  Scope: workspace | Source: skills | Files: 2

### nda

- **contract-drafting** — >
  Scope: workspace | Source: skills | Files: 2

### network

- **debugging-tools** — >
  Scope: workspace | Source: skills | Files: 2

### networking

- **linux-admin** — >
  Scope: workspace | Source: skills | Files: 2

### networking]

- **cloud-security** — >
  Scope: workspace | Source: skills | Files: 2

### no-code

- **no-code-automation** — >
  Scope: workspace | Source: skills | Files: 2

### npm

- **live-dependency-resolver** — >
  Scope: workspace | Source: skills | Files: 2

### nps

- **customer-research** — >
  Scope: workspace | Source: skills | Files: 2
- **support-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### nrr

- **customer-success-playbook** — >
  Scope: workspace | Source: skills | Files: 2

### nx

- **monorepo-management** — >
  Scope: workspace | Source: skills | Files: 2

### object-detection

- **computer-vision** — >
  Scope: workspace | Source: skills | Files: 2

### object-pool

- **game-design-patterns** — >
  Scope: workspace | Source: skills | Files: 2

### objections

- **sales-playbook** — >
  Scope: workspace | Source: skills | Files: 2

### observability

- **sentry** — >
  Scope: workspace | Source: skills | Files: 3
- **signoz** — >
  Scope: workspace | Source: skills | Files: 3
- **supaguard** — >
  Scope: workspace | Source: skills | Files: 2

### observability]

- **backend-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### offer-letters

- **employment-law** — >
  Scope: workspace | Source: skills | Files: 2

### okrs

- **performance-management** — >
  Scope: workspace | Source: skills | Files: 2

### on-call

- **incident-management** — >
  Scope: workspace | Source: skills | Files: 2

### onboarding

- **codedocs** — >
  Scope: workspace | Source: skills | Files: 2
- **developer-experience** — >
  Scope: workspace | Source: skills | Files: 2

### open-source

- **ip-management** — >
  Scope: workspace | Source: skills | Files: 2

### openapi

- **api-design** — >
  Scope: workspace | Source: skills | Files: 2

### opentelemetry

- **signoz** — >
  Scope: workspace | Source: skills | Files: 3

### opportunity-trees

- **product-discovery** — >
  Scope: workspace | Source: skills | Files: 2

### optimization

- **database-engineering** — >
  Scope: workspace | Source: skills | Files: 2
- **performance-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### orchestration

- **docker-kubernetes** — >
  Scope: workspace | Source: skills | Files: 2

### orchestration]

- **ai-agent-design** — >
  Scope: workspace | Source: skills | Files: 2
- **meta-repo** — >
  Scope: workspace | Source: skills | Files: 3

### orchestrator

- **video-creator** — >
  Scope: workspace | Source: skills | Files: 2

### outbound

- **sales-playbook** — >
  Scope: workspace | Source: skills | Files: 2

### owasp

- **appsec-owasp** — >
  Scope: workspace | Source: skills | Files: 2

### p-and-l

- **financial-reporting** — >
  Scope: workspace | Source: skills | Files: 2

### pacing]

- **video-scriptwriting** — >
  Scope: workspace | Source: skills | Files: 2

### package-management

- **live-dependency-resolver** — >
  Scope: workspace | Source: skills | Files: 2

### packaging

- **pricing-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### palette

- **pixel-art-sprites** — >
  Scope: workspace | Source: skills | Files: 2

### pandas

- **data-science** — >
  Scope: workspace | Source: skills | Files: 3

### panes

- **cmux** — >
  Scope: workspace | Source: skills | Files: 3

### patents

- **ip-management** — >
  Scope: workspace | Source: skills | Files: 2

### patterns

- **microservices** — >
  Scope: workspace | Source: skills | Files: 2
- **refactoring-patterns** — >
  Scope: workspace | Source: skills | Files: 2
- **regex-mastery** — >
  Scope: workspace | Source: skills | Files: 2

### pay-bands

- **compensation-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### pci-dss

- **regulatory-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### performance

- **load-testing** — >
  Scope: workspace | Source: skills | Files: 2
- **sentry** — >
  Scope: workspace | Source: skills | Files: 3

### persistence

- **second-brain** — >
  Scope: workspace | Source: skills | Files: 2

### personalization]

- **second-brain** — >
  Scope: workspace | Source: skills | Files: 2

### personas

- **customer-research** — >
  Scope: workspace | Source: skills | Files: 2

### persuasion

- **copywriting** — >
  Scope: workspace | Source: skills | Files: 2

### physics]

- **unity-development** — >
  Scope: workspace | Source: skills | Files: 2

### pip

- **live-dependency-resolver** — >
  Scope: workspace | Source: skills | Files: 2

### pipeline

- **crm-management** — >
  Scope: workspace | Source: skills | Files: 2

### pipelines

- **ci-cd-pipelines** — >
  Scope: workspace | Source: skills | Files: 2

### pips]

- **performance-management** — >
  Scope: workspace | Source: skills | Files: 2

### pitch-deck

- **presentation-design** — >
  Scope: workspace | Source: skills | Files: 3
- **startup-fundraising** — >
  Scope: workspace | Source: skills | Files: 2

### pivot-tables

- **spreadsheet-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### pixel-perfect

- **figma-to-code** — >
  Scope: workspace | Source: skills | Files: 2

### planning

- **absolute-brainstorm** — >
  Scope: workspace | Source: skills | Files: 2
- **ai-agent-design** — >
  Scope: workspace | Source: skills | Files: 2

### planning]

- **product-strategy** — >
  Scope: workspace | Source: skills | Files: 2
- **project-execution** — >
  Scope: workspace | Source: skills | Files: 2

### play-store]

- **android-kotlin** — >
  Scope: workspace | Source: skills | Files: 2

### playtesting

- **game-balancing** — >
  Scope: workspace | Source: skills | Files: 2

### playwright

- **supaguard** — >
  Scope: workspace | Source: skills | Files: 2

### plg]

- **growth-hacking** — >
  Scope: workspace | Source: skills | Files: 2

### plugins

- **vim-neovim** — >
  Scope: workspace | Source: skills | Files: 2

### policies]

- **employment-law** — >
  Scope: workspace | Source: skills | Files: 2

### polyrepo

- **meta-repo** — >
  Scope: workspace | Source: skills | Files: 3

### positioning

- **brand-strategy** — >
  Scope: workspace | Source: skills | Files: 2
- **competitive-analysis** — >
  Scope: workspace | Source: skills | Files: 2

### post-mortem

- **internal-docs** — >
  Scope: workspace | Source: skills | Files: 2

### post-mortems

- **incident-management** — >
  Scope: workspace | Source: skills | Files: 2

### pr-review

- **address-pr-comments** — >
  Scope: workspace | Source: skills | Files: 2

### pricing

- **api-monetization** — >
  Scope: workspace | Source: skills | Files: 3
- **proposal-writing** — >
  Scope: workspace | Source: skills | Files: 2

### prioritization

- **product-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### production]

- **llm-app-development** — >
  Scope: workspace | Source: skills | Files: 2
- **video-creator** — >
  Scope: workspace | Source: skills | Files: 2

### profiling

- **debugging-tools** — >
  Scope: workspace | Source: skills | Files: 2
- **performance-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### programmatic-seo

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2

### programmatic-video

- **video-creator** — >
  Scope: workspace | Source: skills | Files: 2

### programmatic-video]

- **remotion-video** — >
  Scope: workspace | Source: skills | Files: 2

### progression

- **game-balancing** — >
  Scope: workspace | Source: skills | Files: 2

### prompt-injection

- **skill-audit** — >
  Scope: workspace | Source: skills | Files: 2

### prompts]

- **cli-design** — >
  Scope: workspace | Source: skills | Files: 2

### public-speaking]

- **presentation-design** — >
  Scope: workspace | Source: skills | Files: 3

### pull-request

- **address-pr-comments** — >
  Scope: workspace | Source: skills | Files: 2

### qbr

- **account-management** — >
  Scope: workspace | Source: skills | Files: 2

### qualification

- **lead-scoring** — >
  Scope: workspace | Source: skills | Files: 2

### quality]

- **code-review-mastery** — >
  Scope: workspace | Source: skills | Files: 2
- **test-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### r-and-d-credits

- **tax-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### rag

- **mastra** — >
  Scope: workspace | Source: skills | Files: 3
- **prompt-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### ramp

- **onboarding** — >
  Scope: workspace | Source: skills | Files: 2

### rate-limiting

- **api-monetization** — >
  Scope: workspace | Source: skills | Files: 3

### react

- **remotion-video** — >
  Scope: workspace | Source: skills | Files: 2

### real-time]

- **real-time-streaming** — >
  Scope: workspace | Source: skills | Files: 2

### rebase

- **git-advanced** — >
  Scope: workspace | Source: skills | Files: 2

### reconciliation

- **bookkeeping-automation** — >
  Scope: workspace | Source: skills | Files: 2

### refactoring

- **absolute-simplify** — >
  Scope: workspace | Source: skills | Files: 2
- **clean-code** — >
  Scope: workspace | Source: skills | Files: 2

### referral

- **growth-hacking** — >
  Scope: workspace | Source: skills | Files: 2

### registry

- **skill-forge** — >
  Scope: workspace | Source: skills | Files: 1

### reliability

- **chaos-engineering** — >
  Scope: workspace | Source: skills | Files: 2
- **site-reliability** — >
  Scope: workspace | Source: skills | Files: 2

### remotion

- **video-creator** — >
  Scope: workspace | Source: skills | Files: 2

### rendering

- **remotion-video** — >
  Scope: workspace | Source: skills | Files: 2

### renewals]

- **account-management** — >
  Scope: workspace | Source: skills | Files: 2

### reputation]

- **email-deliverability** — >
  Scope: workspace | Source: skills | Files: 2

### resilience

- **chaos-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### resolution-time

- **support-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### rest

- **api-design** — >
  Scope: workspace | Source: skills | Files: 2
- **api-testing** — >
  Scope: workspace | Source: skills | Files: 2

### retention

- **employee-engagement** — >
  Scope: workspace | Source: skills | Files: 2
- **growth-hacking** — >
  Scope: workspace | Source: skills | Files: 2

### retention]

- **customer-success-playbook** — >
  Scope: workspace | Source: skills | Files: 2
- **product-analytics** — >
  Scope: workspace | Source: skills | Files: 2

### retrospective]

- **agile-scrum** — >
  Scope: workspace | Source: skills | Files: 2

### review-comments]

- **address-pr-comments** — >
  Scope: workspace | Source: skills | Files: 2

### reviews

- **performance-management** — >
  Scope: workspace | Source: skills | Files: 2

### rfp

- **proposal-writing** — >
  Scope: workspace | Source: skills | Files: 2

### rice

- **product-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### risk-management

- **project-execution** — >
  Scope: workspace | Source: skills | Files: 2

### roadmap

- **product-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### roi]

- **sales-enablement** — >
  Scope: workspace | Source: skills | Files: 2

### rollout

- **product-launch** — >
  Scope: workspace | Source: skills | Files: 2

### room

- **android-kotlin** — >
  Scope: workspace | Source: skills | Files: 2

### rtl

- **localization-i18n** — >
  Scope: workspace | Source: skills | Files: 2

### rubrics

- **interview-design** — >
  Scope: workspace | Source: skills | Files: 2
- **technical-interviewing** — >
  Scope: workspace | Source: skills | Files: 2

### runbook

- **internal-docs** — >
  Scope: workspace | Source: skills | Files: 2

### runbooks

- **incident-management** — >
  Scope: workspace | Source: skills | Files: 2
- **technical-writing** — >
  Scope: workspace | Source: skills | Files: 2

### saas

- **pricing-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### saas-agreement

- **contract-drafting** — >
  Scope: workspace | Source: skills | Files: 2

### salesforce

- **crm-management** — >
  Scope: workspace | Source: skills | Files: 2

### scaffolding

- **skill-forge** — >
  Scope: workspace | Source: skills | Files: 1

### scalability

- **system-design** — >
  Scope: workspace | Source: skills | Files: 2

### scd]

- **data-warehousing** — >
  Scope: workspace | Source: skills | Files: 2

### scene-detection

- **video-analyzer** — >
  Scope: workspace | Source: skills | Files: 2

### scheduling

- **social-media-scheduling** — >
  Scope: workspace | Source: skills | Files: 2

### schema

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2
- **database-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### screen-reader]

- **accessibility-wcag** — >
  Scope: workspace | Source: skills | Files: 2

### screening

- **recruiting-ops** — >
  Scope: workspace | Source: skills | Files: 2

### scripting

- **shell-scripting** — >
  Scope: workspace | Source: skills | Files: 2

### scriptwriting

- **video-production** — >
  Scope: workspace | Source: skills | Files: 2

### scrum

- **agile-scrum** — >
  Scope: workspace | Source: skills | Files: 2

### sdk-design

- **developer-experience** — >
  Scope: workspace | Source: skills | Files: 2

### sdk]

- **developer-advocacy** — >
  Scope: workspace | Source: skills | Files: 2
- **x-twitter-scraper** — >
  Scope: workspace | Source: skills | Files: 2

### search

- **nlp-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### search-intent

- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### secrets

- **cloud-security** — >
  Scope: workspace | Source: skills | Files: 2

### security

- **backend-engineering** — >
  Scope: workspace | Source: skills | Files: 2
- **penetration-testing** — >
  Scope: workspace | Source: skills | Files: 2
- **security-incident-response** — >
  Scope: workspace | Source: skills | Files: 2
- **web3-smart-contracts** — >
  Scope: workspace | Source: skills | Files: 2

### security]

- **linux-admin** — >
  Scope: workspace | Source: skills | Files: 2

### segmentation

- **computer-vision** — >
  Scope: workspace | Source: skills | Files: 2

### self-serve]

- **analytics-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### self-service

- **knowledge-base** — >
  Scope: workspace | Source: skills | Files: 2

### semantic-layer

- **analytics-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### seo

- **absolute-marketing** — >
  Scope: workspace | Source: skills | Files: 2
- **video-production** — >
  Scope: workspace | Source: skills | Files: 2

### seo-content

- **content-marketing** — >
  Scope: workspace | Source: skills | Files: 2

### serp-analysis

- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### serverless

- **cloud-aws** — >
  Scope: workspace | Source: skills | Files: 2
- **cloud-gcp** — >
  Scope: workspace | Source: skills | Files: 2
- **edge-computing** — >
  Scope: workspace | Source: skills | Files: 2

### services]

- **microservices** — >
  Scope: workspace | Source: skills | Files: 2

### session-replay]

- **posthog** — >
  Scope: workspace | Source: skills | Files: 3

### sfx

- **video-audio-design** — >
  Scope: workspace | Source: skills | Files: 2

### shaders

- **unity-development** — >
  Scope: workspace | Source: skills | Files: 2

### shell

- **linux-admin** — >
  Scope: workspace | Source: skills | Files: 2
- **shell-scripting** — >
  Scope: workspace | Source: skills | Files: 2

### skills

- **skill-audit** — >
  Scope: workspace | Source: skills | Files: 2

### sla

- **customer-support-ops** — >
  Scope: workspace | Source: skills | Files: 2

### slides

- **presentation-design** — >
  Scope: workspace | Source: skills | Files: 3

### slo]

- **observability** — >
  Scope: workspace | Source: skills | Files: 2

### smart-contracts

- **web3-smart-contracts** — >
  Scope: workspace | Source: skills | Files: 2

### snowflake

- **data-warehousing** — >
  Scope: workspace | Source: skills | Files: 2

### soc2

- **regulatory-compliance** — >
  Scope: workspace | Source: skills | Files: 2

### social-media

- **x-twitter-scraper** — >
  Scope: workspace | Source: skills | Files: 2

### solid

- **clean-code** — >
  Scope: workspace | Source: skills | Files: 2

### sound-design

- **game-audio** — >
  Scope: workspace | Source: skills | Files: 2

### sourcing

- **recruiting-ops** — >
  Scope: workspace | Source: skills | Files: 2

### sow

- **proposal-writing** — >
  Scope: workspace | Source: skills | Files: 2

### spark

- **data-pipelines** — >
  Scope: workspace | Source: skills | Files: 2

### spatial-audio

- **game-audio** — >
  Scope: workspace | Source: skills | Files: 2

### spec-writing

- **absolute-brainstorm** — >
  Scope: workspace | Source: skills | Files: 2

### spf

- **email-deliverability** — >
  Scope: workspace | Source: skills | Files: 2

### split

- **cmux** — >
  Scope: workspace | Source: skills | Files: 3

### sprint

- **agile-scrum** — >
  Scope: workspace | Source: skills | Files: 2

### sprites

- **pixel-art-sprites** — >
  Scope: workspace | Source: skills | Files: 2

### sql

- **database-engineering** — >
  Scope: workspace | Source: skills | Files: 2

### sql-injection

- **appsec-owasp** — >
  Scope: workspace | Source: skills | Files: 2

### sql]

- **lead-scoring** — >
  Scope: workspace | Source: skills | Files: 2

### stakeholder-communication

- **project-execution** — >
  Scope: workspace | Source: skills | Files: 2

### stakeholder-mapping

- **account-management** — >
  Scope: workspace | Source: skills | Files: 2

### star-schema

- **data-warehousing** — >
  Scope: workspace | Source: skills | Files: 2

### startup

- **startup-fundraising** — >
  Scope: workspace | Source: skills | Files: 2

### state-machine

- **game-design-patterns** — >
  Scope: workspace | Source: skills | Files: 2

### statistics

- **data-science** — >
  Scope: workspace | Source: skills | Files: 3

### status-pages

- **incident-management** — >
  Scope: workspace | Source: skills | Files: 2

### story-mapping

- **user-stories** — >
  Scope: workspace | Source: skills | Files: 2

### storyboard

- **video-scriptwriting** — >
  Scope: workspace | Source: skills | Files: 2

### storytelling

- **brand-strategy** — >
  Scope: workspace | Source: skills | Files: 2
- **presentation-design** — >
  Scope: workspace | Source: skills | Files: 3

### strace

- **debugging-tools** — >
  Scope: workspace | Source: skills | Files: 2

### strategy

- **test-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### strategy]

- **competitive-analysis** — >
  Scope: workspace | Source: skills | Files: 2

### stream-processing

- **real-time-streaming** — >
  Scope: workspace | Source: skills | Files: 2

### streaming]

- **data-pipelines** — >
  Scope: workspace | Source: skills | Files: 2

### stress-testing

- **load-testing** — >
  Scope: workspace | Source: skills | Files: 2

### stripe

- **api-monetization** — >
  Scope: workspace | Source: skills | Files: 3

### subagent

- **cmux** — >
  Scope: workspace | Source: skills | Files: 3

### supply-chain

- **skill-audit** — >
  Scope: workspace | Source: skills | Files: 2

### surveys

- **customer-research** — >
  Scope: workspace | Source: skills | Files: 2
- **employee-engagement** — >
  Scope: workspace | Source: skills | Files: 2
- **posthog** — >
  Scope: workspace | Source: skills | Files: 3

### swift

- **ios-swift** — >
  Scope: workspace | Source: skills | Files: 2

### swiftui

- **ios-swift** — >
  Scope: workspace | Source: skills | Files: 2

### swot

- **competitive-analysis** — >
  Scope: workspace | Source: skills | Files: 2

### synthetic-monitoring

- **supaguard** — >
  Scope: workspace | Source: skills | Files: 2

### sysadmin

- **linux-admin** — >
  Scope: workspace | Source: skills | Files: 2

### system-design]

- **technical-interviewing** — >
  Scope: workspace | Source: skills | Files: 2

### systemd

- **linux-admin** — >
  Scope: workspace | Source: skills | Files: 2

### tailwind

- **absolute-ui** — >
  Scope: workspace | Source: skills | Files: 3

### talent-acquisition]

- **recruiting-ops** — >
  Scope: workspace | Source: skills | Files: 2

### talks

- **developer-advocacy** — >
  Scope: workspace | Source: skills | Files: 2

### tdd

- **clean-code** — >
  Scope: workspace | Source: skills | Files: 2
- **test-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### team-health]

- **employee-engagement** — >
  Scope: workspace | Source: skills | Files: 2

### technical-seo

- **absolute-seo** — >
  Scope: workspace | Source: skills | Files: 2

### term-sheets

- **startup-fundraising** — >
  Scope: workspace | Source: skills | Files: 2

### terminal

- **cli-design** — >
  Scope: workspace | Source: skills | Files: 2
- **react-ink** — >
  Scope: workspace | Source: skills | Files: 3

### termination

- **employment-law** — >
  Scope: workspace | Source: skills | Files: 2

### test-pyramid

- **test-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### testing

- **cypress-testing** — >
  Scope: workspace | Source: skills | Files: 2
- **mobile-testing** — >
  Scope: workspace | Source: skills | Files: 2
- **playwright-testing** — >
  Scope: workspace | Source: skills | Files: 3
- **vite-plus** — >
  Scope: workspace | Source: skills | Files: 3

### testing]

- **jest-vitest** — >
  Scope: workspace | Source: skills | Files: 2
- **ux-research** — >
  Scope: workspace | Source: skills | Files: 2

### text-processing

- **nlp-engineering** — >
  Scope: workspace | Source: skills | Files: 2
- **regex-mastery** — >
  Scope: workspace | Source: skills | Files: 2

### thumbnails

- **video-production** — >
  Scope: workspace | Source: skills | Files: 2

### ticketing]

- **customer-support-ops** — >
  Scope: workspace | Source: skills | Files: 2

### tilesets

- **pixel-art-sprites** — >
  Scope: workspace | Source: skills | Files: 2

### tls

- **cryptography** — >
  Scope: workspace | Source: skills | Files: 2

### toil

- **site-reliability** — >
  Scope: workspace | Source: skills | Files: 2

### tool-use

- **ai-agent-design** — >
  Scope: workspace | Source: skills | Files: 2

### toolchain

- **vite-plus** — >
  Scope: workspace | Source: skills | Files: 3

### tooling]

- **monorepo-management** — >
  Scope: workspace | Source: skills | Files: 2

### total-rewards

- **compensation-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### tracing

- **observability** — >
  Scope: workspace | Source: skills | Files: 2
- **sentry** — >
  Scope: workspace | Source: skills | Files: 3
- **signoz** — >
  Scope: workspace | Source: skills | Files: 3

### trademarks

- **ip-management** — >
  Scope: workspace | Source: skills | Files: 2

### transfer-pricing

- **tax-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### transformation]

- **refactoring-patterns** — >
  Scope: workspace | Source: skills | Files: 2

### translation

- **localization-i18n** — >
  Scope: workspace | Source: skills | Files: 2

### tri-surface

- **keyword-research** — >
  Scope: workspace | Source: skills | Files: 2

### triage

- **customer-support-ops** — >
  Scope: workspace | Source: skills | Files: 2

### tts

- **video-audio-design** — >
  Scope: workspace | Source: skills | Files: 2

### tui

- **react-ink** — >
  Scope: workspace | Source: skills | Files: 3

### turborepo

- **monorepo-management** — >
  Scope: workspace | Source: skills | Files: 2

### tutorials]

- **technical-writing** — >
  Scope: workspace | Source: skills | Files: 2

### twitter

- **x-twitter-scraper** — >
  Scope: workspace | Source: skills | Files: 2

### typescript

- **mastra** — >
  Scope: workspace | Source: skills | Files: 3

### ui]

- **frontend-developer** — >
  Scope: workspace | Source: skills | Files: 2

### uikit

- **ios-swift** — >
  Scope: workspace | Source: skills | Files: 2

### unit-testing

- **jest-vitest** — >
  Scope: workspace | Source: skills | Files: 2

### uptime

- **supaguard** — >
  Scope: workspace | Source: skills | Files: 2

### usability

- **ux-research** — >
  Scope: workspace | Source: skills | Files: 2

### use-cases

- **clean-architecture** — >
  Scope: workspace | Source: skills | Files: 2

### user-interviews

- **ux-research** — >
  Scope: workspace | Source: skills | Files: 2

### ux

- **absolute-ui** — >
  Scope: workspace | Source: skills | Files: 3

### validation

- **data-quality** — >
  Scope: workspace | Source: skills | Files: 2
- **regex-mastery** — >
  Scope: workspace | Source: skills | Files: 2

### validation]

- **product-discovery** — >
  Scope: workspace | Source: skills | Files: 2

### valuation

- **financial-modeling** — >
  Scope: workspace | Source: skills | Files: 2

### variance-analysis

- **budgeting-planning** — >
  Scope: workspace | Source: skills | Files: 2

### venture]

- **startup-fundraising** — >
  Scope: workspace | Source: skills | Files: 2

### version-check]

- **live-dependency-resolver** — >
  Scope: workspace | Source: skills | Files: 2

### version-control]

- **git-advanced** — >
  Scope: workspace | Source: skills | Files: 2

### video

- **remotion-video** — >
  Scope: workspace | Source: skills | Files: 2

### video-analysis

- **video-analyzer** — >
  Scope: workspace | Source: skills | Files: 2

### video-script

- **video-scriptwriting** — >
  Scope: workspace | Source: skills | Files: 2

### vim

- **vim-neovim** — >
  Scope: workspace | Source: skills | Files: 2

### viral-loops

- **growth-hacking** — >
  Scope: workspace | Source: skills | Files: 2

### vision

- **product-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### visual-regression

- **playwright-testing** — >
  Scope: workspace | Source: skills | Files: 3

### visualization

- **data-science** — >
  Scope: workspace | Source: skills | Files: 3

### vitest

- **jest-vitest** — >
  Scope: workspace | Source: skills | Files: 2

### voice-tone

- **brand-strategy** — >
  Scope: workspace | Source: skills | Files: 2

### vulnerability-assessment

- **penetration-testing** — >
  Scope: workspace | Source: skills | Files: 2

### war-rooms]

- **incident-management** — >
  Scope: workspace | Source: skills | Files: 2

### wcag

- **accessibility-wcag** — >
  Scope: workspace | Source: skills | Files: 2

### web-performance

- **frontend-developer** — >
  Scope: workspace | Source: skills | Files: 2

### web3

- **web3-smart-contracts** — >
  Scope: workspace | Source: skills | Files: 2

### well-architected]

- **cloud-aws** — >
  Scope: workspace | Source: skills | Files: 2

### win-themes

- **proposal-writing** — >
  Scope: workspace | Source: skills | Files: 2

### workflows

- **mastra** — >
  Scope: workspace | Source: skills | Files: 3

### workspaces

- **monorepo-management** — >
  Scope: workspace | Source: skills | Files: 2

### worktrees

- **git-advanced** — >
  Scope: workspace | Source: skills | Files: 2

### wwise

- **game-audio** — >
  Scope: workspace | Source: skills | Files: 2

### xss

- **appsec-owasp** — >
  Scope: workspace | Source: skills | Files: 2

### yaml

- **video-scriptwriting** — >
  Scope: workspace | Source: skills | Files: 2

### youtube

- **video-production** — >
  Scope: workspace | Source: skills | Files: 2

### zero-trust

- **cloud-security** — >
  Scope: workspace | Source: skills | Files: 2

### zsh

- **shell-scripting** — >
  Scope: workspace | Source: skills | Files: 2

### Untagged

- **agentic-eval** — |
- **skill-creator** — Create new skills, modify and improve existing skills, and measure skill performance. Use when users want to create a skill from scratch, edit, or optimize an existing skill, run evals to test a skill, benchmark skill performance with variance analysis, or optimize a skill's description for better triggering accuracy.
- **caveman** — >
- **mcp-builder** — Guide for creating high-quality MCP (Model Context Protocol) servers that enable LLMs to interact with external services through well-designed tools. Use when building MCP servers to integrate external APIs or services, whether in Python (FastMCP) or Node/TypeScript (MCP SDK).
- **prompt-engineer** — "Expert in designing effective prompts for LLM-powered applications. Masters prompt structure, context management, output formatting, and prompt evaluation. Use when: prompt engineering, system prompt, few-shot, chain of thought, prompt design."
- **deploy-to-vercel** — Deploy applications and websites to Vercel. Use when the user requests deployment actions like "deploy my app", "deploy and give me the link", "push this live", or "create a preview deployment".
- **vercel-cli-with-tokens** — Deploy and manage projects on Vercel using token-based authentication. Use when working with Vercel CLI using access tokens rather than interactive login — e.g. "deploy to vercel", "set up vercel", "add environment variables to vercel".
- **vercel-composition-patterns** — 
- **vercel-react-best-practices** — React and Next.js performance optimization guidelines from Vercel Engineering. This skill should be used when writing, reviewing, or refactoring React/Next.js code to ensure optimal performance patterns. Triggers on tasks involving React components, Next.js pages, data fetching, bundle optimization, or performance improvements.
- **vercel-react-native-skills** — 
- **vercel-react-view-transitions** — Guide for implementing smooth, native-feeling animations using React's View Transition API (`<ViewTransition>` component, `addTransitionType`, and CSS view transition pseudo-elements). Use this skill whenever the user wants to add page transitions, animate route changes, create shared element animations, animate enter/exit of components, animate list reorder, implement directional (forward/back) navigation animations, or integrate view transitions in Next.js. Also use when the user mentions view transitions, `startViewTransition`, `ViewTransition`, transition types, or asks about animating between UI states in React without third-party animation libraries.
- **web-design-guidelines** — Review UI code for Web Interface Guidelines compliance. Use when asked to "review my UI", "check accessibility", "audit design", "review UX", or "check my site against best practices".
- **agentic-eval** — |
- **skill-creator** — Create new skills, modify and improve existing skills, and measure skill performance. Use when users want to create a skill from scratch, edit, or optimize an existing skill, run evals to test a skill, benchmark skill performance with variance analysis, or optimize a skill's description for better triggering accuracy.
- **absolute-human** — >
- **caveman** — >
- **deploy-to-vercel** — Deploy applications and websites to Vercel. Use when the user requests deployment actions like "deploy my app", "deploy and give me the link", "push this live", or "create a preview deployment".
- **find-skills** — Helps users discover and install agent skills when they ask questions like "how do I do X", "find a skill for X", "is there a skill that can...", or express interest in extending capabilities. This skill should be used when the user is looking for functionality that might exist as an installable skill.
- **mcp-builder** — Guide for creating high-quality MCP (Model Context Protocol) servers that enable LLMs to interact with external services through well-designed tools. Use when building MCP servers to integrate external APIs or services, whether in Python (FastMCP) or Node/TypeScript (MCP SDK).
- **prompt-engineer** — "Expert in designing effective prompts for LLM-powered applications. Masters prompt structure, context management, output formatting, and prompt evaluation. Use when: prompt engineering, system prompt, few-shot, chain of thought, prompt design."
- **vercel-cli-with-tokens** — Deploy and manage projects on Vercel using token-based authentication. Use when working with Vercel CLI using access tokens rather than interactive login — e.g. "deploy to vercel", "set up vercel", "add environment variables to vercel".
- **vercel-composition-patterns** — 
- **vercel-react-best-practices** — React and Next.js performance optimization guidelines from Vercel Engineering. This skill should be used when writing, reviewing, or refactoring React/Next.js code to ensure optimal performance patterns. Triggers on tasks involving React components, Next.js pages, data fetching, bundle optimization, or performance improvements.
- **vercel-react-native-skills** — 
- **vercel-react-view-transitions** — Guide for implementing smooth, native-feeling animations using React's View Transition API (`<ViewTransition>` component, `addTransitionType`, and CSS view transition pseudo-elements). Use this skill whenever the user wants to add page transitions, animate route changes, create shared element animations, animate enter/exit of components, animate list reorder, implement directional (forward/back) navigation animations, or integrate view transitions in Next.js. Also use when the user mentions view transitions, `startViewTransition`, `ViewTransition`, transition types, or asks about animating between UI states in React without third-party animation libraries.
- **web-design-guidelines** — Review UI code for Web Interface Guidelines compliance. Use when asked to "review my UI", "check accessibility", "audit design", "review UX", or "check my site against best practices".

## How to Invoke a Skill

In your orchestrator or agent prompt:
```
You have access to the following skills. Use them when relevant:
- agentic-eval: |
- skill-creator: Create new skills, modify and improve existing skills, and measure skill performance. Use when users
- caveman: >
- mcp-builder: Guide for creating high-quality MCP (Model Context Protocol) servers that enable LLMs to interact wi
- prompt-engineer: "Expert in designing effective prompts for LLM-powered applications. Masters prompt structure, conte
- deploy-to-vercel: Deploy applications and websites to Vercel. Use when the user requests deployment actions like "depl
- vercel-cli-with-tokens: Deploy and manage projects on Vercel using token-based authentication. Use when working with Vercel 
- vercel-composition-patterns: 
- vercel-react-best-practices: React and Next.js performance optimization guidelines from Vercel Engineering. This skill should be 
- vercel-react-native-skills: 
- vercel-react-view-transitions: Guide for implementing smooth, native-feeling animations using React's View Transition API (`<ViewTr
- web-design-guidelines: Review UI code for Web Interface Guidelines compliance. Use when asked to "review my UI", "check acc
- agentic-eval: |
- skill-creator: Create new skills, modify and improve existing skills, and measure skill performance. Use when users
- a2a-protocol: >
- a2ui: >
- absolute-brainstorm: >
- absolute-human: >
- absolute-marketing: >
- absolute-seo: >
- ... and 162 more
```

To fully load a skill, read its SKILL.md and supporting files.

Discovered 182 skills
Sources scanned:
  EXISTS: /home/raed/Projects/air-mentor-ui/.windsurf/skills
  EXISTS: /home/raed/.codeium/windsurf/skills
  NOT FOUND: /home/raed/Projects/air-mentor-ui/.claude/skills
  EXISTS: /home/raed/.claude/skills
  NOT FOUND: /home/raed/Projects/air-mentor-ui/.cursor/skills
  NOT FOUND: /home/raed/.cursor/skills
  EXISTS: /home/raed/Projects/air-mentor-ui/.agents/skills
  EXISTS: /home/raed/.agents/skills

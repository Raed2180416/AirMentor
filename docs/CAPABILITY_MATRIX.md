...
| Course Leader guided demo reality loop | partial / demo-only — local panel now mounts on the Course Leader dashboard and demonstrates one selected proof-queue student through attendance edit → academic recompute-risk route → risk/card refresh → intervention resolution → next-stage advance; verified by `tests/demo-reality-loop.test.tsx`, `tests/academic-route-pages.test.tsx`, `tests/api-client.test.ts`, `air-mentor-api/tests/academic-proof-routes.test.ts`, and focused Nix Firefox `tests-e2e/specs/guided-demo-reality-loop.spec.ts`; full six-semester demo proof still not claimed for this panel | local P5/P9 demo affordance; full ladder → P9 |

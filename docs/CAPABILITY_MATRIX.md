...
| Active proof run + new provisioning collision rule | partial — demo-scoped proof-run activation and P5-D seeded provisioning no longer deactivate or replace the global active run; broader provisioning collision UX still pending | `air-mentor-api/tests/demo-isolation.test.ts` → P5/P9 |
| Reset Demo Workspace | works for P5 seeded-demo scope — schema drop, demo-session invalidation, cloned academic/proof artifact cleanup, generated playback artifact cleanup, and global-row non-interference verified by `air-mentor-api/tests/demo-isolation.test.ts`; route-specific browser walkthrough not rerun | C10 closed for backend P5-D; browser evidence → P9 |
| Provisioning preview / dry run | works for estimate-only preview via `air-mentor-api/tests/demo-isolation.test.ts` | C11 → P5 |
| Demo data isolation (`demoWorkspaceId`) | partial — schema registry/reset, demo session pointer isolation, scoped academic bootstrap guard, scoped academic snapshot builder, seeded per-demo academic/proof artifact provisioning, generated demo playback checkpoints/projections, demo proof-run scope propagation, demo/global activation non-interference, proof-run admin route scope guard, academic proof route scope guard, student-shell proof-run scope guard, reassessment proof-run scope guard, and academic checkpoint playback scope guard verified by `air-mentor-api/tests/demo-isolation.test.ts`, `air-mentor-api/tests/proof-control-plane-seeded-bootstrap-service.test.ts`, `tests/demo-workspace-pointer.test.ts`, and `tests/api-client.test.ts`; broad physical schema routing for every table, P5-D browser walkthrough, and multi-program demo templates remain deferred | C1 backend P5-D closed; browser/multi-program → P9/P6 |
| Course Leader guided demo reality loop | partial / demo-only — local panel now mounts on the Course Leader dashboard and demonstrates one selected proof-queue student through attendance edit → academic recompute-risk route → risk/card refresh → intervention resolution → next-stage advance; verified by `tests/demo-reality-loop.test.tsx`, `tests/academic-route-pages.test.tsx`, `tests/api-client.test.ts`, `air-mentor-api/tests/academic-proof-routes.test.ts`, focused Nix Firefox `tests-e2e/specs/guided-demo-reality-loop.spec.ts`, and closeout proof pack in `audit-map/32-reports/guided-demo-reality-loop-closeout-2026-05-11.md`; full six-semester proof is claimed for core demo surfaces via `full-demo-ladder`, not as a dedicated six-stage panel-specific scenario | local P5/P9 demo affordance; multi-program generality → P6 |
...
| Demo isolation regression test (global rows untouched) | partial — backend schema/reset/session-pointer/scoped-bootstrap/scoped-academic-snapshot/proof-run activation/admin-scope/academic-proof-scope/student-shell-scope/reassessment-scope/checkpoint-scope regressions, seeded demo workspace provisioning with nonzero demo checkpoints/observed states/risk assessments, and frontend pointer/header tests covered; fresh local Firefox P5/P9 focused browser pack passed 12/12 on 2026-05-10 via `flow-1`, `flow-2`, `flow-4`, `flow-5`, `flow-6`, `flow-8`, `flow-9`, `flow-10`, `flow-11`; fresh all-spec Firefox regression passed 20/20 in 42.2m on ports 4100/5174; P5-D route-specific browser walkthrough and performance baseline remain pending | H1 backend P5-D closed; browser/perf → P9 |

...
| Demo isolation regression test (global rows untouched) | partial — backend schema/reset/session-pointer/scoped-bootstrap/scoped-academic-snapshot/proof-run activation/admin-scope/academic-proof-scope/student-shell-scope regressions and frontend pointer/header tests covered; full browser walkthrough pending | H1 → P5/P9 |
